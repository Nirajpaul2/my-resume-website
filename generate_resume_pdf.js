...
async function createExactResumePDF() {
  ...
  const margin = 36;
  const width = 612; // 8.5 in
  const height = 792; // 11 in

  let page = pdfDoc.addPage([width, height]);
  let y = height - margin;

  const drawHeader = () => {
    page.drawText("NIRAJ PAUL", { x: margin, y, size: 18, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
    y -= 16;
    page.drawText("SENIOR iOS DEVELOPER | 11+ YEARS EXPERIENCE", { x: margin, y, size: 9.5, font: fontBold, color: rgb(0.3, 0.3, 0.35) });
    y -= 14;
    page.drawText("Bangalore, India  |  +91-9942419837  |  nirajpaul.ios@gmail.com  |  LinkedIn  |  GitHub", { x: margin, y, size: 8.5, font: fontRegular, color: rgb(0.0, 0.34, 0.7) });
    y -= 16;
  };
  ...
  const drawSectionHeading = (title) => {
    checkNewPage(26);
    page.drawText(title, { x: margin, y, size: 10.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
    y -= 4;
    page.drawLine({ start: { x: margin, y }, end: { x: width - margin, y }, thickness: 0.75, color: rgb(0.2, 0.2, 0.2) });
    y -= 12;
  };
  ...
  const drawParagraph = (text, isBold = false, size = 8.5, indent = 0) => {
  ...
  const drawJobHeader = (leftText, rightText) => {
    checkNewPage(16);
    page.drawText(leftText, { x: margin, y, size: 8.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
    const rightWidth = fontBold.widthOfTextAtSize(rightText, 8.5);
    page.drawText(rightText, { x: width - margin - rightWidth, y, size: 8.5, font: fontBold, color: rgb(0.2, 0.2, 0.2) });
    y -= 11;
  };

  const drawBullet = (text, indent = 10) => {
    const font = fontRegular;
    const maxWidth = width - margin * 2 - indent - 8;
    const lines = wrapText(text, maxWidth, 8.5, font);
    if (lines.length === 0) return;

    checkNewPage(12);
    page.drawText("•", { x: margin + indent, y, size: 8.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
    page.drawText(lines[0], { x: margin + indent + 8, y, size: 8.5, font, color: rgb(0.15, 0.15, 0.15) });
    y -= 11;

    for (let i = 1; i < lines.length; i++) {
      checkNewPage(12);
      page.drawText(lines[i], { x: margin + indent + 8, y, size: 8.5, font, color: rgb(0.15, 0.15, 0.15) });
      y -= 11;
    }
  };
  ...
  drawBullet("Integrations & Analytics: Juspay Payment, Stripe, PayPal, Braintree (Merchant Integrations), PubNub, XMPP, Socket.io, Firebase FCM & Analytics, NPAW, Comscore, RudderStack, OneTrust");
  y -= 6;
  ...
  // Job 1
  drawJobHeader("Senior iOS Developer (Contract) | Wipro Limited (Client: Tegna Inc.)", "May 2025 – March 2026");
  ...
  // Job 2
  drawJobHeader("Senior iOS Developer | NeoSoft Technology", "June 2021 – April 2025");
  ...
  drawBullet("Sony Pictures Network - Built and maintained the SonyLIV AppleTV app (Sony Pictures Network) in Swift and SwiftUI, implementing custom UI components and optimizing data fetching for high-concurrency live sports and show streaming, reducing content load time by 20%");
  ...
  // Job 3
  drawJobHeader("Senior iOS Developer | Sakha Global (Client: Spectra Logic)", "October 2020 – June 2021");
  ...
  // Job 4
  drawJobHeader("Senior iOS Developer | Aquarious Technology", "June 2017 – July 2020");
  ...
  // Job 5
  drawJobHeader("iOS Developer | Unified Infotech", "December 2016 – April 2017");
  ...
  // Job 6
  drawJobHeader("iOS Developer | Brainium Information Technologies", "January 2016 – December 2016");
  ...
  // Job 7
  drawJobHeader("Junior iOS Developer | WebTech Software Solution", "November 2014 – December 2015");
  ...
  drawJobHeader("Bachelor of Technology in Information Technology", "April 2010 – July 2014");
  page.drawText("B.P. Poddar Institute of Management and Technology", { x: margin, y: y + 2, size: 8.5, font: fontItalic, color: rgb(0.3, 0.3, 0.3) });

  const pdfBytes = await pdfDoc.save();
  const outputPath = path.join(__dirname, "public", "Niraj_Paul_Senior_iOS_Developer_Resume.pdf");
  fs.writeFileSync(outputPath, pdfBytes);
  console.log("PDF generated matching exact layout at:", outputPath);
}

createExactResumePDF().catch(console.error);
